...
- 기록 삭제: 교사 대시보드 "최근 누적 기록" 표에서 행별 삭제(테스트 데이터 정리용). 삭제는 교사 계정만 가능하며 `firestore.rules` 의 `simple_responses` 삭제 규칙을 콘솔에 다시 게시해야 동작합니다.
- 학생 이메일(`student_email`)은 수집·시트 내보내기에는 남지만 **대시보드 화면에는 표시하지 않습니다.**
