...
교사는 **두 가지 방법** 중 하나로 **언제든** 켜고 끌 수 있습니다.

1. **비밀코드** (`js/config.js` 의 `ACTIVATION_CODE`): 로그인 없이 코드만 입력하면 켜기/끄기.
2. **구글 교사 로그인**: 교사 계정으로 로그인하면 코드 없이 바로 토글.

각 화면 위치:

- **학생 메인 화면**(`index.html`): 비활성 화면 아래 "선생님 전용 · 사이트 켜기/끄기" 에서 코드 입력(또는 구글 로그인). 학생(비교사)에게는 보이지 않습니다.
- **교사 대시보드**(`index.html?teacher=1`): 상단 "사이트 상태" 카드에서 코드 입력(또는 구글 로그인).

> 상태는 Firestore `app_config/site` (`active: true/false`) 에 저장되어 모든 학생 기기에 실시간 반영됩니다.
> 비밀코드는 클라이언트에 노출되는 편의용 코드라 강한 보안은 아닙니다(기존 교사 코드와 동일 수준). 강한 보안이 필요하면 구글 로그인 방식만 사용하고 규칙을 좁히세요.

- 상태는 Firestore `app_config/site` 문서(`active: true/false`)에 저장되어 모든 학생 기기에 **실시간 반영**됩니다.
- 문서가 없거나 `active` 가 `true` 가 아니면 기본값은 **꺼짐**입니다.
- 이 기능을 쓰려면 `firestore.rules` 의 `app_config` 규칙(공개 읽기 + `active` 토글만 쓰기 허용)을 Firebase 콘솔에 **한 번 게시**해야 합니다.
