...
현재 학생 제출은 Firestore Rules에서도 사이트 활성 상태와 입력 잠금 상태를 확인합니다. 즉, `app_config/site.active == true`이고 `app_config/lesson.inputEnabled`가 `false`가 아닐 때만 학생 제출이 허용됩니다. 이 규칙을 수정했다면 Rules를 반드시 다시 게시해야 합니다.
