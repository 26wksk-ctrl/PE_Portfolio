...
- **학급(반)**: 학생이 학급을 한 번 고르면 그 학급이 기억되어(보정값이 없으면 **본인 최근 기록의 학급**), 다음에 로그인하면 학생 화면에서 **자동으로 선택**됩니다. 교사는 같은 표의 **"학급 수정"** 에서 학생의 학급을 바로잡을 수 있고, 저장하면 `students/{uid}.session_id`·`class_id` 에 기록되어 **학생 화면 자동 선택과 새 기록**에 반영됩니다.
  - 학급 표시 해석 순서: **`students/{uid}`(교사 보정) → 본인 최근 기록의 학급**.
