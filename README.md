...
- **편집 시 재조회 안 함**: 학생 이름·학급을 저장할 때 대시보드 전체를 다시 읽지 않고 화면 데이터만 메모리에서 갱신합니다. (여러 명을 연속 수정해도 읽기가 늘지 않음)
...
구글 계정 이름(`displayName`)이 실명과 다르거나 학급이 잘못 입력된 경우를 위해, 교사가 실명·학급을 따로 저장하는 컬렉션입니다. 문서 형태: `{ name, session_id, class_id }`.
...
- **학급(반)**: 학생이 학급을 한 번 제출하면 그 학급이 **기기에 기억**되어, 다음에 로그인하면 학생 화면에서 **자동으로 선택**됩니다. 교사는 같은 표의 **"학급 수정"** 에서 학생의 학급을 바로잡을 수 있고, 저장하면 `students/{uid}.session_id`·`class_id` 에 기록되어 **학생 화면 자동 선택과 새 기록**에 반영됩니다.
  - 학급 자동 선택 해석 순서: **`students/{uid}`(교사 보정, 1읽기) → 기기 캐시(`localStorage`, 읽기 0)**. 서버 기록을 매번 스캔하지 않아 Firestore 읽기를 아낍니다.
  - **지난 기록의 `class_id` 는 그대로 보존**됩니다(통계 일관성 + 데이터 보존). 즉 학급 보정은 학생 화면과 앞으로의 기록에만 적용됩니다.
