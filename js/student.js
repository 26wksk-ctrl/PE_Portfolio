...
let lessonRenderSig = null; // 수업 설정 중 '오늘 기록' 폼 렌더에 영향을 주는 값들의 서명(불필요한 재렌더 방지)
...
export function initStudent() {
  ...
  // 수업 설정(입력 잠금 inputEnabled·활동/질문 옵션 등)을 실시간 구독한다.
  // 사이트 상태처럼 실시간으로 반영해, 교사가 입력 잠금을 풀거나 설정을 바꾸면
  // 학생이 새로고침하거나 사이트를 껐다 켜지 않아도 즉시 반영된다.
  // (이전에는 buildStudentApp 안에서 1회만 읽어, 사이트를 재토글해야만 갱신되는 문제가 있었다.)
  watchLessonSettings(applyLessonSettings);
...
function buildStudentApp() {
  renderStudentShell();
  loadInitial(SESSION_ID_PARAM);  // 현재 ACTIVE 로 '오늘 기록' 폼을 그린다(수업 설정은 watchLessonSettings 가 실시간 유지)
...
// '오늘 기록' 폼 렌더에 영향을 주는 수업 설정 값들의 서명. 같으면 폼을 다시 그리지 않아
// 학생이 입력 중이던 내용을 불필요하게 초기화하지 않는다.(잠금/옵션이 실제로 바뀐 경우에만 재렌더)
function lessonRenderSignature(s) {
  return JSON.stringify([
    s.inputEnabled, s.recordType, s.feedbackMode, s.activity, s.coreQuestion, s.unit,
    s.activityOptions, s.goalOptions, s.methodOptions, s.feedbackOptions,
    s.resultOptions, s.nextTryOptions, s.selFocus
  ]);
}

// 교사 수업 설정(app_config/lesson)의 실시간 스냅샷을 ACTIVE 에 반영한다.
// watchLessonSettings 콜백. raw 가 null 이면(문서 없음/읽기 실패) 기본값으로 fail-open 한다.
function applyLessonSettings(raw) {
  ...
  // 폼이 이미 만들어져 있고(사이트 켜짐) 렌더에 영향을 주는 값이 바뀐 경우에만 다시 그린다.
  const sig = lessonRenderSignature(ACTIVE);
  if (document.getElementById('mainFormCard') && sig !== lessonRenderSig) renderMainForm();
  lessonRenderSig = sig;
...
// 보통은 이메일 자동 연결로 이 화면을 건너뛰지만, 자동 연결이 안 되면 학번+이름으로 직접 연결한다.
function renderProfileRegistration(reason) {
  const el = document.getElementById('studentCard');
  if (!el) return;

  // 자동 연결이 실패한 이유에 맞춘 안내 문구
  const reasonMsg = {
