...
function stopDashboardWatch() {
  if (dashboardUnsub) {
    try { dashboardUnsub(); } catch { /* ignore */ }
  }
  dashboardUnsub = null;
  dashboardWatchKey = '';
}

function dashboardParamsKey(params) {
...
async function loadTeacherDashboard() {
  ...
  // 다시 getDocs/onSnapshot을 시작하면 최초 스냅샷만큼 읽기가 다시 발생할 수 있어서 막는다.
  if (dashboardUnsub && dashboardWatchKey === key && lastDashboard) {
    lastDashboard._rangeLabel = range.label;
    renderTeacherDashboard(lastDashboard);
    showTeacherInfo('이미 이 조건으로 실시간 갱신 중입니다. 새 기록이 들어오면 자동 반영됩니다.');
    return;
  }
  ...
  try {
    dashboardUnsub = watchTeacherDashboardData(params, data => {
...
function renderTeacherDashboard(data) {
  ...
  document.getElementById('dashboardResult').innerHTML = `
    <section class="card">
      ...
      <p class="muted" style="margin-top:8px;">조회 범위: <strong>${escapeHtml(data._rangeLabel || '전체')}</strong>${data.live ? ' · 실시간 갱신 중' : ''}${data.fromCache ? ' · 캐시 표시' : ''}${data.capped ? ` · ⚠️ 데이터가 많아 차트는 최근 ${escapeHtml(data.chartSampleSize)}건 표본만 반영했습니다. 기간을 좁혀 주세요.` : ''}</p>
    </section>
    <section class="card">
      <h2>차시별 주도성 추이</h2>
